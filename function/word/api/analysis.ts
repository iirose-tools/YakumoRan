...
export const analysis = {
  ...
  */
  start (rawq:string, m:any, dir:any, nickname:string) {
    ...
    // 输入替换
    try { wd = replaceWord(q, wd, name, numdata, uid, userName, name, nickname) } catch (err:any) { return err.message }
    // 输出常规替换与判断
    try { wd = condition(wd, tha, uid, dir, userName, q) } catch (err:any) { return err.message }
    // 收集本次干了啥，失败则恢复
    const things:any = {}
    const thingnum = 0
    // 减少物品
    try { wd = loss(wd, tha, dir, uid, things, thingnum, userName) } catch (err:any) { return err.message }
    // 添加物品
    try { wd = add(wd, tha, dir, uid, things, thingnum) } catch (err:any) { return err.message }
    // 最后检测及换行
    try { wd = attribute(wd, tha, dir, uid) } catch (err:any) { return err.message }
