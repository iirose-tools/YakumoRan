...
// 解析触发词
const parsing = (rawq:string, m:any) => {
  let tid:any
  let tname:any
  const numdata = []
  let q = rawq
  ...
  if (tid) {
    ...
    if (tha.indexOf('http') > -1 && config.app.master_uid !== m.uid) { return ' [词库核心] 包含被禁止的标识 ' } // 屏蔽图片
  }

  // 将各种情况的艾特转换为@
  if (q.match(/\s*\[\*([\s\S]+?)\*\]\s*/)) {
    tname = q.match(/\s*\[\*([\s\S]+?)\*\]\s*/)
  }
  ...
  if (tname) {
    ...
    if (name.indexOf('http') > -1 && config.app.master_uid !== m.uid) { return ' [词库核心] 包含被禁止的标识 ' } // 屏蔽图片
  }
...
// 常规替换
const replaceWord = (q:string, wd:string, nam:string, numdata:any, uid:string, userName:string, name:string, nickname:string, tha:any) => {
  // 将$数$替换为数
  while (wd.match(/\$数(.*?)\$/)) {
  ...
  while (wd.match(/\$that\$/)) {
    if (wd.match(/\$that\$/)) {
      const over = wd.match(/\$that\$/)

      if (over) {
        wd = wd.replace(over[0], tha)
      }
    }
  }
...
// 标记不需要解析的区域
const saveValue = (wd:string) => {
  let time = 0
  while (wd.match(/\$\[([\s\S]+?)\]\$/)) {
    if (wd.match(/\$\[([\s\S]+?)\]\$/)) {
      const over = wd.match(/\$\[([\s\S]+?)\]\$/)
      if (over) {
  ...
  */
  start (rawq:string, m:any, dir:any, nickname:string) {
    ...
    if (getword(dir)[rawq]) {
      ...
      logger('Word').info(`[@${m.username}@]  /  [*${m.uid}*]  已触发词库   【${rawq}】`)
    ...
    try {
      ...
      // 输入替换
      wd = replaceWord(q, wd, name, numdata, uid, userName, name, nickname, tha)
  ...
  /**
    ...
    * @return 返回为回复结果
   */
  toPars (wd:string, dir:any, uid:string, userName:string) {
    // 收集减法/加法本次干了啥，失败则恢复这些记录
    const things:any = {}
    const thingnum = 0
    ...
    try {
      ...
      // 初始化解析对象
      const pars = new Pars(wd, '', dir, uid, things, thingnum, userName, '')
